...
def test_database_functionality():
    ...
    try:
        ...
        for onto in reference_ontologies:
            print(f"  - {onto.get('label', 'No label')} ({onto.get('graph_iri', 'No IRI')})")
...
def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ API endpoint test passed - found {len(reference_ontologies)} reference ontologies")
            
            for onto in reference_ontologies:
                print(f"  - {onto.get('label', 'No label')} ({onto.get('graph_iri', 'No IRI')})")
                print(f"    Project: {onto.get('project_name', 'Unknown')}")
        else:
            print(f"❌ API endpoint test failed with status {response.status_code}: {response.text}")
            return False
            
        return True
        
    except requests.exceptions.ConnectionError:
        print("❌ Could not connect to the API server. Make sure the server is running on localhost:8000")
        return False
    except Exception as e:
