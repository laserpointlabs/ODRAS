...
def chunk_document(
    ...
    try:
        ...
        chunking_stats = {
            ...
            'min_chunk_size': min(len(chunk['content']) for chunk in processed_chunks) if processed_chunks else 0,
            'max_chunk_size': max(len(chunk['content']) for chunk in processed_chunks) if processed_chunks else 0,
