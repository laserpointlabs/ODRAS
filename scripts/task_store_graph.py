...
def store_results_in_graph_db(processed_requirements: List[Dict]) -> Dict[str, Any]:
    ...
    try:
        ...
        for req_idx, req in enumerate(processed_requirements):
            print(
                f"Processing requirement {req_idx + 1}/{len(processed_requirements)}: {req['original_requirement']['id']}"
            )
            ...
            triples.extend(
                [
                    ...
                    (
                        f"req:{req_id}",
                        "CATEGORY",
                        req["original_requirement"].get("category", "Unknown"),
                    ),
                    (
                        f"req:{req_id}",
                        "SOURCE_FILE",
                        req["original_requirement"].get("source_file", "Unknown"),
                    ),
                    (
                        f"req:{req_id}",
                    ...
                    (
                        ...
                        str(req["original_requirement"].get("timestamp", time.time())),
                    ),
                ]
            )
                        ...
                        for perf_req in iteration["llm_response"]["performance_requirements"]:
                            ...
                            triples.extend(
                                [
                                    ...
                                    (
                                        f"iter:{iter_id}",
                                        "IDENTIFIES_PERFORMANCE",
                                        perf_id,
                                    ),
                                    (
                                        f"req:{req_id}",
                                        "HAS_PERFORMANCE_REQUIREMENT",
                                        perf_id,
                                    ),
                                ]
                            )
        ...
        if triples:
            ...
            try:
                ...
                print(
                    f"Successfully created {len(unique_nodes)} nodes and {len(triples)} relationships"
                )
        ...
        print(
            f"Created {graph_store_status['nodes_created']} nodes and {graph_store_status['relationships_created']} relationships in Neo4j"
        )
