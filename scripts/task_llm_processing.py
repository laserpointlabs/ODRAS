...
async def process_requirements_with_llm(
    ...
    for req_idx, req in enumerate(requirements_list):
        ...
        for i in range(iterations):
            ...
            try:
                ...
                llm_result = _analyze_requirement_with_llm(
                    llm_team, req, i, llm_provider, llm_model
                )
        ...
        processed_requirements.append(
            {
                ...
                "success_rate": (
                    len(successful_results) / len(req_results) if req_results else 0.0
                ),
            }
        )
        ...
        print(
            f"  Average confidence: {avg_confidence:.3f}, Success rate: {len(successful_results)}/{len(req_results)}"
        )
    ...
    return {
        ...
        "llm_results": {
            ...
            "average_processing_time_per_requirement": (
                total_processing_time / len(processed_requirements)
                if processed_requirements
                else 0.0
            ),
            "successful_iterations_total": sum(
                len([r for r in req["iterations"] if r["status"] == "success"])
                for req in processed_requirements
            ),
            "failed_iterations_total": sum(
                len([r for r in req["iterations"] if r["status"] == "error"])
                for req in processed_requirements
            ),
        },
    }
...
def _analyze_requirement_with_llm(
    llm_team: LLMTeam, requirement: Dict, iteration: int, provider: str, model: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"error": str(e), "prompt": prompt, "provider": provider, "model": model}
...
def _calculate_iteration_confidence(
    iteration: int, total_iterations: int, llm_result: Dict
) -> float:
