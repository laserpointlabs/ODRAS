...
def check_postgres_data(conn):
    ...
    try:
        with conn.cursor() as cur:
            ...
            users = cur.fetchall()
            floating_data['users'] = [{'user_id': u[0], 'username': u[1], 'display_name': u[2], 'is_admin': u[3]} for u in users]
            
            # Check projects
            cur.execute("SELECT project_id, name, description, created_by, is_active FROM public.projects")
            projects = cur.fetchall()
            floating_data['projects'] = [{'project_id': p[0], 'name': p[1], 'description': p[2], 'created_by': p[3], 'is_active': p[4]} for p in projects]
            
            # Check project members
            cur.execute("SELECT project_id, user_id, role FROM public.project_members")
            members = cur.fetchall()
            floating_data['project_members'] = [{'project_id': m[0], 'user_id': m[1], 'role': m[2]} for m in members]
            
            # Check ontologies registry
            cur.execute("SELECT id, project_id, graph_iri, label, role, is_reference FROM public.ontologies_registry")
            ontologies = cur.fetchall()
            floating_data['ontologies_registry'] = [{'id': o[0], 'project_id': o[1], 'graph_iri': o[2], 'label': o[3], 'role': o[4], 'is_reference': o[5]} for o in ontologies]
            
            # Check file metadata
            cur.execute("SELECT file_id, filename, project_id, created_by, is_deleted FROM file_storage.file_metadata")
            files = cur.fetchall()
            floating_data['file_metadata'] = [{'file_id': f[0], 'filename': f[1], 'project_id': f[2], 'created_by': f[3], 'is_deleted': f[4]} for f in files]
            
            # Check extraction jobs
            cur.execute("SELECT job_id, project_id, file_id, status FROM public.extraction_jobs")
            jobs = cur.fetchall()
            floating_data['extraction_jobs'] = [{'job_id': j[0], 'project_id': j[1], 'file_id': j[2], 'status': j[3]} for j in jobs]
            
            # Check requirements
            cur.execute("SELECT requirement_id, project_id, file_id, text FROM public.requirements LIMIT 10")
            requirements = cur.fetchall()
            floating_data['requirements'] = [{'requirement_id': r[0], 'project_id': r[1], 'file_id': r[2], 'text': r[3][:100] + '...' if len(r[3]) > 100 else r[3]} for r in requirements]
            
            # Check ontology changes
            cur.execute("SELECT change_id, user_id, change_type, change_description FROM public.ontology_changes LIMIT 10")
            changes = cur.fetchall()
            floating_data['ontology_changes'] = [{'change_id': c[0], 'user_id': c[1], 'change_type': c[2], 'change_description': c[3]} for c in changes]
...
def check_fuseki_data():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            datasets = datasets_data.get('datasets', []) if isinstance(datasets_data, dict) else datasets_data
            ...
            for dataset in datasets:
                ...
                try:
                    ...
                    if graph_response.status_code == 200:
                        ...
                        graphs = [binding['g']['value'] for binding in results.get('results', {}).get('bindings', [])]
                        floating_graphs.extend([{'dataset': dataset_name, 'graph': graph} for graph in graphs])
