                        ...
                        for graph_iri in graphs:
                            if dry_run:
                                ...
                                if delete_response.status_code in [200, 204]:
                                    ...
                                    print(f"      ✗ Failed to delete graph: {graph_iri} (status: {delete_response.status_code})")
            ...
            for collection in collections:
                ...
                if dry_run:
                    ...
                    if info_response.status_code == 200:
                        ...
                        print(f"  Would delete collection '{collection_name}' with {points_count} points")
                else:
                    # Delete collection
                    ...
                    if delete_response.status_code in [200, 202]:
                        ...
                        print(f"  ✗ Failed to delete collection: {collection_name} (status: {delete_response.status_code})")
        else:
...
def main():
    ...
    parser.add_argument('--postgres-only', action='store_true',
                       help='Only clean up PostgreSQL data')
    parser.add_argument('--fuseki-only', action='store_true',
                       help='Only clean up Fuseki data')
    parser.add_argument('--qdrant-only', action='store_true',
                       help='Only clean up Qdrant data')
