    ...
    try:
        ...
        if diagram is not None:
            ...
            process_element_ids = {
                ...
                if elem.get("id") and elem.tag.endswith("sequenceFlow") is False
            }
            shape_element_ids = {
                shape.get("bpmnElement") for shape in shapes if shape.get("bpmnElement")
            }
            ...
            if missing_shapes:
                result["missing_elements"].extend(
                    [f"Missing shape for: {id}" for id in missing_shapes]
                )
                result["valid"] = False
            ...
            if missing_edges:
                result["missing_elements"].extend(
                    [f"Missing edge for: {id}" for id in missing_edges]
                )
                result["valid"] = False
