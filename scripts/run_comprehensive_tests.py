...
class ODRASTestRunner:
    ...
    def get_test_suites(self) -> List[TestSuite]:
        ...
        return [
            ...
            TestSuite(
                name="api_validation",
                command=["python", "scripts/validate_all_endpoints.py", f"--base-url={self.base_url}"],
                description="Comprehensive API endpoint validation",
                critical=True
            ),
            TestSuite(
                name="code_quality",
                command=["flake8", "backend/", "--count", "--select=E9,F63,F7,F82", "--show-source", "--statistics"],
            ...
            TestSuite(
                ...
                command=["python", "-c", "from backend.main import app; print('✅ Imports successful')"],
...
async def main():
    ...
    try:
        ...
        if report["summary"]["critical_failures"] > 0:
            console.print(f"\n❌ {report['summary']['critical_failures']} critical test failures!", style="red")
            sys.exit(1)
        else:
