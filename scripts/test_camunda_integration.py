...
async def test_document_upload():
    ...
    files = {"file": ("test_requirements.txt", test_content.encode(), "text/plain")}

    data = {"iterations": "3", "llm_provider": "openai", "llm_model": "gpt-4o-mini"}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for deployment in deployments:
                    print(
                        f"   - {deployment.get('name', 'Unknown')} ({deployment.get('id', 'No ID')})"
                    )
            else:
