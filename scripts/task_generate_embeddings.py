...
def generate_embeddings(chunks_data: List[Dict], embedding_model: str = None, batch_size: int = 10) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for batch_idx in range(0, len(texts), batch_size):
            ...
            try:
                ...
                for i, embedding in enumerate(batch_embeddings):
                    ...
                    if metadata_idx < len(chunk_metadata):
                        embedding_record = {
                            ...
                            'content_preview': texts[metadata_idx][:200] + '...' if len(texts[metadata_idx]) > 200 else texts[metadata_idx],
                ...
                print(f"Generated embeddings for batch {(batch_idx // batch_size) + 1}/{total_batches}")
                
            except Exception as e:
                embedding_result['errors'].append(f"Failed to generate embeddings for batch {batch_idx // batch_size}: {str(e)}")
                continue
        ...
        embedding_result['embedding_stats'] = {
            ...
            'average_embedding_time': processing_time / total_embeddings if total_embeddings > 0 else 0,
...
def validate_embeddings(embeddings: List[Dict]) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(unique_dimensions) > 1:
            validation_result['warnings'].append(f"Inconsistent embedding dimensions: {unique_dimensions}")
        ...
        if zero_vectors > 0:
            validation_result['warnings'].append(f"Found {zero_vectors} zero vectors out of {len(embeddings)}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(sys.argv[1]):
            ...
            text = sys.argv[1]
            chunks_data = [
                {'chunk_id': 'chunk_0', 'chunk_index': 0, 'content': text}
            ]
