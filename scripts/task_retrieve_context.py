...
def retrieve_context(processed_query: Dict, search_parameters: Dict = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if HAS_EMBEDDING_SERVICE and HAS_QDRANT_SERVICE:
            ...
            result = perform_mock_search(query_text, primary_terms, max_results, min_similarity)  # Use mock for now
        else:
...
async def perform_async_vector_search(query_text: str, terms: List[str], max_results: int, min_similarity: float) -> Dict[str, Any]:
    ...
    try:
        ...
        for chunk in search_results:
            ...
            processed_chunk = {
                ...
                'content': chunk_data.get('text', ''),  # Note: existing RAG uses 'text' not 'content'
...
def perform_mock_search(query_text: str, terms: List[str], max_results: int, min_similarity: float) -> Dict[str, Any]:
    ...
    for i, chunk in enumerate(mock_chunks):
        ...
        similarity = (overlap / total_unique * 0.9 + 0.1) if total_unique > 0 else 0.5  # Add base score
