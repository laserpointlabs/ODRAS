...
class RAGComparativeTest:
            ...
            for i in range(max_wait):
                ...
                try:
                    status_response = requests.get(
                        f"{self.camunda_base}/process-instance/{process_id}", timeout=3
                    )
            ...
            try:
                ...
                if hist_vars_response.status_code == 200:
                    ...
                    if chunks_found == 0:
                        try:
                            context_data = (
                                json.loads(reranked_context)
                                if isinstance(reranked_context, str)
                                else []
                            )
                    ...
                    print(
                        f"   📊 Debug - Context length: {len(reranked_context) if reranked_context else 0}"
                    )
    ...
    def generate_summary_report(self):
        ...
        # Calculate averages
        similarities = [
            r["comparison"]["similarity"]
            for r in self.results
            if r["comparison"]["both_successful"]
        ]
        ...
        print(
            f"   Tests Passed: {successful_both}/{total_tests} ({successful_both/total_tests*100:.1f}%)"
        )
        ...
        for i, result in enumerate(self.results, 1):
            ...
            print(
                f"   {i}. {status} | Sim: {similarity:.3f} | Terms: {coverage:.3f} | {query[:40]}..."
            )
            ...
            if not comp["both_successful"]:
                ...
                print(
                    f"      Original: {'✅' if orig_success else '❌'} | BPMN: {'✅' if bpmn_success else '❌'}"
                )
        ...
        if successful_both == total_tests:
            ...
            print(
                f"   ⚠️  PARTIAL: {successful_both}/{total_tests} tests passed (60%+ success rate)"
            )
            print(f"   🔧 Several issues need attention")
        else:
            print(
                f"   ❌ POOR: Only {successful_both}/{total_tests} tests passed (<60% success rate)"
            )
