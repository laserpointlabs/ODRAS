...
def load_classes_to_ontology(ontology_name, classes, token):
    ...
    if response.status_code == 200:
        ...
        print(f"❌ Failed to load classes into {ontology_name}: {response.status_code} - {response.text}")
        return False
...
def verify_classes_loaded(ontology_name, token):
    ...
    if response.status_code == 200:
        ...
        print(f"❌ Failed to query classes in {ontology_name}: {response.status_code} - {response.text}")
        return False
