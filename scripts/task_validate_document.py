...
def validate_document(document_path: str, filename: str, mime_type: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if file_size > max_size:
            validation_result['validation_result'] = 'failure'
            validation_result['validation_errors'].append(f"File too large: {file_size} bytes (max {max_size})")
            return validation_result
            ...
            try:
                with open(document_path, 'r', encoding='utf-8') as f:
                    ...
                    if not content.strip():
                        validation_result['validation_result'] = 'failure'
                        validation_result['validation_errors'].append("Text file contains no content")
                        return validation_result
                ...
                try:
                    with open(document_path, 'r', encoding='latin1') as f:
                        ...
                        if not content.strip():
                            validation_result['validation_result'] = 'failure'
                            validation_result['validation_errors'].append("Text file contains no readable content")
                            return validation_result
                except Exception as e:
                    validation_result['validation_result'] = 'failure'
                    validation_result['validation_errors'].append(f"Cannot read text file: {str(e)}")
                    return validation_result
