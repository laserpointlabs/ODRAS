...
class DatabaseSchemaValidator:
    ...
    def validate_migration_files(self) -> ValidationResult:
        ...
        for i, (num, filename) in enumerate(numbered_files):
            expected_num = i + 1
            if num != expected_num:
                warnings.append(f"Migration numbering gap: expected {expected_num:03d}, found {num:03d} in {filename}")
    ...
    def validate_odras_script(self) -> ValidationResult:
        ...
        if extra_in_script:
            warnings.append(f"Extra migrations in odras.sh (files don't exist): {sorted(extra_in_script)}")
...
try:
    ...
    def validate_docker_services(self) -> ValidationResult:
        ...
        try:
            ...
            expected_containers = ['odras_postgres', 'odras_neo4j', 'odras_qdrant', 'odras_fuseki']
            ...
            if len(running_containers) < len(expected_containers):
                ...
                print(f"   ⚠️  Only {len(running_containers)}/{len(expected_containers)} containers running")
            else:
