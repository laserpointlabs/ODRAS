...
class DatabaseSchemaManager:
    ...
    def validate_schema_consistency(self) -> SchemaValidationResult:
        ...
        if not migrations:
            ...
            return SchemaValidationResult(False, errors, warnings, missing_migrations, outdated_migrations)
        ...
        for migration in migrations:
            for dep in migration.dependencies:
                ...
                if dep_version not in found_versions:
                    errors.append(f"Migration {migration.version} depends on {dep_version} which doesn't exist")
        ...
        if self.schema_info_file.exists():
            try:
                ...
                for migration in migrations:
                    if migration.checksum != schema_info.get('migrations', {}).get(migration.filename, {}).get('checksum'):
        ...
        return SchemaValidationResult(is_valid, errors, warnings, missing_migrations, outdated_migrations)
...
def main():
    ...
    parser.add_argument("command", choices=[
        "validate", "update-info", "update-odras", "test", "create", "docs", "status"
