...
def main():
    ...
    r = requests.post(query_url, data={"query": sparql}, headers={"Accept": "application/sparql-results+json"}, timeout=15)
    r.raise_for_status()
    ...
    for g in graphs:
        upd = f"DROP GRAPH <{g}>"
        ru = requests.post(update_url, data=upd.encode("utf-8"), headers={"Content-Type": "application/sparql-update"}, timeout=15)
