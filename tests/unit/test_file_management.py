...
async def _ensure_demo_project(
    client: AsyncClient, token: str, name: str = "Demo Project"
) -> str:
...
async def test_upload_and_list_files():
    ...
    async with AsyncClient(
        ...
        data = {
            "project_id": pid,
            "tags": json.dumps({"docType": "knowledge", "status": "new"}),
        }
        r = await client.post(
...
async def test_start_ingestion_workflow(monkeypatch):
    ...
    async with AsyncClient(
        ...
        r = await client.post(
            "/api/workflows/start", content=json.dumps(body), headers=headers
        )
        assert r.status_code == 200, r.text
        data = r.json()
