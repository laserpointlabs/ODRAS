...
class TestDatabaseSchema:
    ...
    def test_migration_files_exist(self, migrations_dir):
        ...
        for expected in expected_migrations:
            assert expected in actual_migrations, f"Expected migration {expected} not found"
    ...
    def test_migration_files_readable(self, migrations_dir):
        ...
        for file_path in migration_files:
            try:
                ...
                assert len(content.strip()) > 0, f"Migration file {file_path.name} is empty"
    ...
    def test_migration_naming_convention(self, migrations_dir):
        ...
        for file_path in migration_files:
            ...
            assert filename[0:3].isdigit(), f"Migration file {filename} doesn't start with 3 digits"
            # Should have underscore after digits
            assert '_' in filename, f"Migration file {filename} doesn't have underscore after version"
    ...
    def test_migration_sequential_numbering(self, migrations_dir):
        ...
        for i, version in enumerate(versions):
            expected = i
            assert version == expected, f"Migration numbering gap: expected {expected:03d}, found {version:03d}"
    ...
    def test_odras_script_migration_list_complete(self, migrations_dir, odras_script):
        ...
        for migration in migration_files:
            assert migration in migration_section, f"Migration {migration} not found in odras.sh"
    ...
    def test_database_schema_validator_script(self):
        ...
        try:
            ...
            assert result.returncode == 0, f"Database schema validator script failed: {result.stderr}"
    ...
    def test_database_schema_manager_script(self):
        ...
        try:
            ...
            assert result.returncode == 0, f"Database schema manager script failed: {result.stderr}"
        except subprocess.TimeoutExpired:
                ...
                if line.startswith('-- DEPENDS ON:'):
                    ...
                    for dep in deps.split(','):
                        ...
                        if dep and dep not in migration_names:
                            pytest.fail(f"Migration {file_path.name} depends on non-existent migration: {dep}")
    ...
    def test_migration_sql_syntax(self, migrations_dir):
        ...
        for file_path in migration_files:
            ...
            assert has_sql, f"Migration {file_path.name} doesn't contain valid SQL keywords"
    ...
    def test_migration_file_encoding(self, migrations_dir):
        ...
        for file_path in migration_files:
            try:
                ...
                assert '\ufffd' not in content, f"Migration {file_path.name} contains replacement characters (encoding issue)"
    ...
    def test_migration_file_size(self, migrations_dir):
        ...
        for file_path in migration_files:
            ...
            assert size >= 100, f"Migration {file_path.name} is too small ({size} bytes)"
            
            # Check maximum size (not more than 1MB)
            assert size <= 1024 * 1024, f"Migration {file_path.name} is too large ({size} bytes)"
    ...
    def test_migration_comments(self, migrations_dir):
        ...
        for file_path in migration_files:
            ...
            assert content.strip().startswith('--'), f"Migration {file_path.name} doesn't start with comment"
            
            # Check for description comment
            lines = content.split('\n')
            has_description = any('-- Migration' in line or '-- ' in line for line in lines[:10])
            assert has_description, f"Migration {file_path.name} doesn't have proper description comment"
    ...
    def test_migration_rollback_support(self, migrations_dir):
        ...
        for file_path in migration_files:
            ...
            has_rollback = any(keyword in content.upper() for keyword in ['DROP', 'ALTER TABLE', 'DELETE'])
    ...
    def test_migration_foreign_key_consistency(self, migrations_dir):
        ...
        for migration_file, referenced_table in table_references.items():
            ...
            if creating_migration:
                ...
                creating_index = migration_files.index(Path(migrations_dir / creating_migration))
                referencing_index = migration_files.index(Path(migrations_dir / migration_file))
