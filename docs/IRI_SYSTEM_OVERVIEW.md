...
An **IRI (Internationalized Resource Identifier)** is a generalization of a URI that permits characters from the Universal Character Set (Unicode/ISO 10646), including characters from non-Latin scripts such as Arabic, Chinese, Cyrillic, Hebrew, or the symbols of various writing systems.

### RFC Specification
ODRAS follows **RFC 3987** - "Internationalized Resource Identifiers (IRIs)" which defines:
- The syntax and processing of IRIs
- The relationship between IRIs and URIs
- Guidelines for IRI normalization and comparison
- Security considerations for IRI handling

### Key Characteristics
- **Persistent**: IRIs remain stable over time
- **Dereferenceable**: Can be resolved to retrieve resource information
- **Globally Unique**: Each IRI identifies exactly one resource
- **Installation-Specific**: ODRAS IRIs include installation domain authority
...
## 🎯 Why IRI Methods Are Critical

Getting the IRI system right is **fundamental to ODRAS's success** because it provides the foundational infrastructure for managing every artifact, object, and component within the system. IRIs serve as the **universal addressing scheme** that enables:

- **Unified Resource Management**: Every process, file, knowledge asset, ontology, class, object property, data property, project, namespace, and system component gets a unique, persistent identifier
- **Cross-Component Relationships**: IRIs allow us to establish semantic links between ontologies and their classes, projects and their files, knowledge assets and their sources, creating a web of interconnected resources
- **Version Control and Traceability**: Each version of an ontology, class definition, or project artifact can be uniquely identified and referenced, enabling complete audit trails and change management
- **Federated Architecture**: IRIs enable ODRAS installations to reference and share resources across organizational boundaries while maintaining clear ownership and authority
- **Semantic Web Integration**: Proper IRIs make ODRAS resources discoverable and usable by external semantic web tools, SPARQL endpoints, and linked data applications
- **API Consistency**: All ODRAS APIs can use the same IRI-based addressing scheme, creating a consistent interface for accessing any resource type

Without robust IRI methods, ODRAS would be a collection of isolated components rather than an integrated, federated knowledge management system. The IRI system is what transforms ODRAS from a simple file storage system into a **semantic knowledge platform** capable of managing complex relationships between diverse resource types across multiple installations.
