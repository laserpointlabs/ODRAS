 ...
 * Lines 5639-17348 extracted and converted to ES module
 */
...
import { showCreateProjectModal } from '../../core/project-manager.js';
...
// authenticatedFetch already defined above

// Token key (defined once)
...
// Auth/UI gating
// tokenKey already defined above
...
// authenticatedFetch already defined at top of file
  } catch (error) {
...
// End of ontology workbench code


// Setup event listeners for ontology workbench
function setupOntologyEventListeners() {
  // Listen for project selection (emits projectId string)
  subscribeToEvent('project:selected', async (projectId) => {
    console.log('🔷 Project selected event received:', projectId);
    if (projectId) {
      // Get full project from state
      const state = getAppState();
      activeProject = state.activeProject;
      // Load ontologies when project is selected
      await loadProjectOntologies(projectId);
    }
  });
  
  // Also listen to state changes directly
  const checkProject = () => {
    const state = getAppState();
    if (state.activeProject?.projectId && state.activeProject.projectId !== activeProject?.id) {
      activeProject = state.activeProject;
      loadProjectOntologies(state.activeProject.projectId);
    }
  };
  
  // Check periodically for project changes (fallback)
  setInterval(checkProject, 1000);
  
  console.log('✅ Ontology event listeners set up');
}
...
async function loadProjectOntologies(projectId) {
  ...
  try {
    ...
    if (response && response.ontologies && response.ontologies.length > 0) {
      ...
      ensureOntologyInitialized();
      // Update ontology tree - this will be handled by renderTree in project manager
      // For now, just ensure we're ready
    } else {
      console.log('📦 No ontologies found for project');
      // Clear canvas if no ontologies
      if (ontoState.cy) {
        ontoState.cy.elements().remove();
      }
      activeOntologyIri = null;
      updateOntoGraphLabel();
      // Update empty state
      const layoutSection = qs('#ontoLayoutSection');
      const emptyState = qs('#ontoEmpty');
...
export function initializeOntologyWorkbench() {
  console.log('🔷 Initializing Ontology Workbench (pluggable version)...');
  
  try {
    // Create the HTML structure dynamically
    const workbenchContainer = document.getElementById('wb-ontology');
    if (!workbenchContainer) {
      console.error('❌ Ontology workbench container not found');
      return;
    }
    
    // Import and create the HTML structure
    import('./ontology-ui-structure.js').then(({ createOntologyWorkbenchHTML }) => {
      workbenchContainer.innerHTML = createOntologyWorkbenchHTML();
      console.log('✅ Ontology workbench HTML structure created');
      
      // Now initialize the functionality
      ensureOntologyInitialized();
      ...
      console.log('✅ Ontology Workbench initialized (pluggable)');
    }).catch(error => {
      console.error('❌ Failed to load ontology workbench structure:', error);
      // Fallback: basic structure
      workbenchContainer.innerHTML = '<div style="padding: 20px;"><h2>Ontology Workbench</h2><p>Loading...</p></div>';
    });
    
  } catch (error) {
    console.error('❌ Error initializing ontology workbench:', error);
    throw error;
  }
}
...
window.loadProjectOntologies = loadProjectOntologies;
