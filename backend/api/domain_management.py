...
class DomainCreate(BaseModel):
    domain: str = Field(..., description="Domain name (lowercase letters, numbers, hyphens only, 2-50 chars)")
    ...
    def validate_domain(cls, v):
        ...
        if not re.match(r'^[a-z][a-z0-9-]{1,49}$', v):
            raise ValueError('Domain must be lowercase letters, numbers, and hyphens only, start with letter, 2-50 characters')
        return v
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (domain_id, domain.domain, domain.description, domain.owner, admin_user["username"])
                )
                conn.commit()
                ...
                return DomainResponse(
                    id=result[0], domain=result[1], description=result[2], owner=result[3],
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                return [
                    DomainResponse(
                        id=row[0], domain=row[1], description=row[2], owner=row[3],
