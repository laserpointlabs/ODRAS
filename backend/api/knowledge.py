...
class KnowledgeAssetCreate(BaseModel):
    ...
    document_type: str = Field(
        default="unknown", pattern="^(requirements|specification|knowledge|reference|unknown)$"
    )
...
class RAGQueryRequest(BaseModel):
    ...
    question: str = Field(
        ..., description="The question to ask the knowledge base", min_length=3, max_length=500
    )
        ...
        if asset_data.source_file_id:
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    if not cur.fetchone():
                        raise HTTPException(
                            status_code=404, detail="Source file not found or not accessible"
                        )
            finally:
                db._return(conn)
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if not row:
                    raise HTTPException(
                        status_code=404, detail="Knowledge asset not found or not accessible"
                    )
        ...
        try:
            with conn.cursor() as cur:
                ...
                if is_user_admin(user):
                    ...
                    cur.execute(
                        "SELECT ka.* FROM knowledge_assets ka WHERE ka.id = %s", (asset_id,)
                    )
                else:
        ...
        if is_user_admin(user):
            ...
            if search_request.project_id:
                if search_request.project_id in accessible_project_ids:
                    ...
                    raise HTTPException(
                        status_code=403, detail="Not authorized to search in this project"
                    )
            else:
