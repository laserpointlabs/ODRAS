...
class PrefixCreate(BaseModel):
    prefix: str = Field(..., description="Prefix (lowercase letters and numbers only, 2-20 chars, e.g., 'dod', 'industry', 'lockheed')")
    ...
    def validate_prefix(cls, v):
        ...
        if not re.match(r'^[a-z][a-z0-9]{1,19}$', v):
            raise ValueError('Prefix must be lowercase letters and numbers only, start with letter, 2-20 characters')
        return v
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (prefix_id, prefix.prefix, prefix.description, prefix.owner, admin_user["username"])
                )
                conn.commit()
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                return [
                    PrefixResponse(
                        id=row[0], prefix=row[1], description=row[2], owner=row[3],
                        status=row[4], created_at=row[5].isoformat(), updated_at=row[6].isoformat(),
                        created_by=row[7]
                    )
                    for row in results
                ]
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT COUNT(*) FROM namespace_registry WHERE prefix = %s", (result[0],))
