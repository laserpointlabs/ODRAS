...
class NamespaceCreate(BaseModel):
    ...
    type: str = Field(..., description="Namespace type: core, domain, program, project, industry, vocab, shapes, align")
...
async def create_namespace(
    ...
    try:
        ...
        valid_types = ['core', 'domain', 'program', 'project', 'industry', 'vocab', 'shapes', 'align']
        ...
        existing_prefix = await db.fetch_one(
            "SELECT id FROM namespace_registry WHERE prefix = $1",
            namespace.prefix
        )
        if existing_prefix:
            raise HTTPException(
        ...
        await db.execute(
            ...
            """,
            namespace_id, namespace.name, namespace.type, namespace.path, 
            namespace.prefix, namespace.description, namespace.owners
        )
...
async def create_version(
    ...
    try:
        ...
        await db.execute(
            ...
            VALUES ($1, $2, $3, $4, 'draft')
            """,
            version_id, namespace_id, version.version, version_iri
        )
...
async def create_class(
    ...
    try:
        ...
        existing = await db.fetch_one(
            ...
            version_info['id'], class_data.local_name
        )
        if existing:
            raise HTTPException(
                status_code=400,
        ...
        await db.execute(
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
            """,
            class_id, version_info['id'], class_data.local_name, 
            class_data.label, iri, class_data.comment
        )
        ...
        if not sync_success:
            logger.warning(f"Failed to sync class {class_data.local_name} to Fuseki, but database record created")
...
@router.put("/{namespace_id}/versions/{version}/classes/{class_id}", response_model=ClassResponse)
        ...
        if class_update.label is not None or class_update.comment is not None:
            ...
            if namespace_info:
                sync_success = await namespace_sync.update_class_in_fuseki(
                    ...
                    class_update.comment if class_update.comment is not None else existing['comment']
                )
                
                if not sync_success:
                    logger.warning(f"Failed to sync class {existing['local_name']} update to Fuseki")
...
@router.get("/public/namespaces/{namespace_id}/versions/{version}/classes", response_model=List[ClassResponse])
