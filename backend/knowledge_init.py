...
async def initialize_knowledge_management(settings: Settings = None) -> bool:
    ...
    try:
        ...
        if successful_components == total_components:
            logger.info(
                f"🎉 Knowledge Management system initialized successfully! ({successful_components}/{total_components} components)"
            )
            return True
        else:
            logger.warning(
                f"⚠️ Knowledge Management system partially initialized ({successful_components}/{total_components} components)"
            )
            return (
                successful_components >= total_components // 2
            )  # Consider success if majority of components work
