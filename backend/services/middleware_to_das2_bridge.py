"""
DAS2 Middleware Bridge - Simple Event Capture
...
class DAS2MiddlewareBridge:
    """
    Simple middleware bridge for DAS2
    Only captures events if project thread exists
    """

    def __init__(self, das2_engine):
        self.das2_engine = das2_engine
        logger.info("DAS2 Middleware Bridge initialized")
    ...
    async def capture_semantic_event(
        ...
        try:
            ...
            project_thread = await self.das2_engine.project_manager.get_project_thread_by_project_id(project_id)
            ...
            await self.das2_engine.project_manager.capture_project_event(
                ...
                event_data={
                    ...
                    "captured_by": "das2_middleware"
                }
            )

            logger.info(f"DAS2: Captured event '{semantic_action}' for project {project_id}")
            return True

        except Exception as e:
            logger.error(f"DAS2 event capture failed: {e}")
            return False
    ...
    async def _get_user_id_from_username(self, username: str) -> Optional[str]:
        ...
        try:
            ...
            db_service = self.das2_engine.rag_service.db_service  # Reuse existing service
