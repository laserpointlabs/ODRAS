        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to sync class to Fuseki: {response.status_code} - {response.text}")
                    return False
    ...
    async def sync_namespace_to_fuseki(
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    logger.info(f"Successfully synced namespace {namespace_path}/{version} with {len(classes)} classes to Fuseki")
                    return True
                else:
                    logger.error(f"Failed to sync namespace to Fuseki: {response.status_code} - {response.text}")
                    return False
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    for binding in data.get('results', {}).get('bindings', []):
                        ...
                        local_name = class_iri.split('#')[-1] if '#' in class_iri else class_iri.split('/')[-1]
                    ...
                    logger.info(f"Retrieved {len(classes)} classes from Fuseki for {namespace_path}/{version}")
                    return classes
                else:
                    logger.error(f"Failed to get classes from Fuseki: {response.status_code} - {response.text}")
                    return []
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if insert_response.status_code == 200:
                    ...
                    logger.error(f"Failed to update class in Fuseki: {insert_response.status_code} - {insert_response.text}")
                    return False
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to delete class from Fuseki: {response.status_code} - {response.text}")
                    return False
