...
class KnowledgeTransformationService:
    ...
    async def create_processing_job(
        self, asset_id: str, job_type: str, metadata: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    async def store_chunks_in_database(
        self, asset_id: str, chunks: List[DocumentChunk], embedding_model: str
    ) -> List[str]:
        """
    ...
    async def generate_and_store_embeddings(
        ...
        try:
            ...
            for i, (chunk, chunk_id, embedding) in enumerate(zip(chunks, chunk_ids, embeddings)):
                ...
                vectors_data.append(
                    {"id": qdrant_point_id, "vector": embedding, "payload": payload}
                )
    ...
    async def update_asset_processing_stats(self, asset_id: str, stats: Dict[str, Any]):
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    if result:
                        ...
                        )  # Try filename first, then original_filename
...
def get_knowledge_transformation_service(
...
async def transform_file_to_knowledge_asset(
    file_id: str, project_id: str, processing_options: Optional[Dict[str, Any]] = None
) -> str:
    """
