...
class IngestionWorker:
    ...
    async def ingest_files(self, file_ids: List[str], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        chunking = self._parse_chunking_params(params.get("chunking") if isinstance(params, dict) else None)
        embedding = self._parse_embedding_params(params.get("embedding") if isinstance(params, dict) else None)
        ...
        for file_id in file_ids or []:
            try:
                ...
                result = await self._process_one(file_id=file_id, chunking=chunking, embedding=embedding)
    ...
    async def _process_one(self, file_id: str, chunking: ChunkingParams, embedding: EmbeddingParams) -> Dict[str, Any]:
        """Process a single file and return detailed results."""
        try:
            meta = await self.storage.get_file_metadata(file_id)
            if not meta:
                return {"success": False, "error": "File metadata not found", "chunks_created": 0, "embeddings_created": 0}
                
            obj = await self.storage.retrieve_file(file_id)
            if not obj or "content" not in obj:
                return {"success": False, "error": "File content not found", "chunks_created": 0, "embeddings_created": 0}
            ...
            if not text:
                return {"success": False, "error": "No text content extracted", "chunks_created": 0, "embeddings_created": 0}
            
            # Create chunks
            chunks = self._chunk_text(text, chunking)
            if not chunks:
                return {"success": False, "error": "No chunks created", "chunks_created": 0, "embeddings_created": 0}
            
            logger.info(f"Created {len(chunks)} chunks for file {file_id}")
            
            # Generate embeddings
            try:
                embedder = self.embedding_service.get_embedder(embedding.modelId, embedding.config or {})
                ...
                logger.info(f"Generated {len(embeddings)} embeddings for file {file_id} using model {embedding.modelId}")
                
            except Exception as e:
                logger.error(f"Embedding generation failed for file {file_id}: {e}")
                return {"success": False, "error": f"Embedding failed: {str(e)}", "chunks_created": len(chunks), "embeddings_created": 0}
            ...
            try:
                ...
                return {"success": False, "error": f"Vector store failed: {str(e)}", "chunks_created": len(chunks), "embeddings_created": len(embeddings)}
            ...
            return {"success": False, "error": f"Unexpected error: {str(e)}", "chunks_created": 0, "embeddings_created": 0}
