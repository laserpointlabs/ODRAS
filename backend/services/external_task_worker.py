...
class ExternalTaskWorker:
        ...
        try:
            ...
            for topic_name in self.task_handlers.keys():
                topics.append({
                    ...
                    "variables": ["document_content", "document_filename", "document_mime_type", 
                                 "parsed_content", "chunks_created", "embeddings_generated",
                                 "knowledge_content", "knowledge_format", "knowledge_metadata",
                                 "processed_knowledge", "similarity_threshold", "search_scope",
                                 "user_query", "query_metadata", "processed_query", "search_parameters",
                                 "retrieved_chunks", "reranked_context", "context_quality_score",
    ...
    def handle_failure(self, task_id: str, error_message: str, error_details: str):
        ...
        try:
            ...
            if response.status_code != 204:
                logger.error(f"Failed to report task failure {task_id}: {response.status_code} - {response.text}")
    ...
    def handle_rerank_context(self, variables: Dict) -> Dict:
        ...
        if retrieved_chunks:
            ...
            avg_score = sum(chunk.get('similarity_score', 0) for chunk in reranked_chunks) / len(reranked_chunks)
        else:
            reranked_chunks = []
            avg_score = 0.0
