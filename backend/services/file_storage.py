...
class FileMetadata:
    ...
    def __init__(self, file_id: str, filename: str, content_type: str, size: int, 
                 hash_md5: str, hash_sha256: str, storage_path: str, 
                 created_at: datetime, updated_at: datetime, 
                 project_id: Optional[str] = None, tags: Optional[Dict] = None):
    ...
    async def store_file(self, file_id: str, content: bytes, metadata: FileMetadata) -> bool:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                """, (
                    file_id, metadata.filename, metadata.content_type, metadata.size,
                    metadata.hash_md5, metadata.hash_sha256, metadata.storage_path,
                    metadata.project_id, str(metadata.tags), metadata.created_at, metadata.updated_at
                ))
...
class FileStorageService:
    ...
    def _initialize_backend(self) -> StorageBackend:
        ...
        if backend_type == "minio":
            if not MINIO_AVAILABLE:
                raise RuntimeError("MinIO backend requested but 'minio' client is not installed. Install with: pip install minio")
            try:
            ...
            if not POSTGRES_AVAILABLE:
                raise RuntimeError("PostgreSQL backend requested but client is not installed. Install with: pip install psycopg2-binary")
            try:
    ...
    async def store_file(self, content: bytes, filename: str, 
        ...
        try:
            ...
            if success:
                ...
                return {
                    "success": False,
                    "error": "Failed to store file in backend"
                }
    ...
    async def list_files(self, project_id: Optional[str] = None, 
