...
class AutoScriptExecutor:
    ...
    async def execute_complete_workflow(
        self, file_id: str, project_id: str, processing_options: Dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            step2_result = await self._run_script(
            ...
            step4_result = await self._run_script(
            ...
            step5_result = await self._run_script(
                "step_store_vector_chunks.py", [file_id, knowledge_asset_id]
            )
            if not step5_result.get("success"):
                raise ValueError(f"Vector storage failed: {step5_result.get('error')}")

            logger.info(
                f"✅ Complete workflow executed successfully: Knowledge asset {knowledge_asset_id}"
            )
