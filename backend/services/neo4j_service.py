    ...
    def create_relationship(
        ...
        try:
            with self.driver.session() as session:
                ...
                from_match = f"(from:{from_label} {{id: $from_id}})" if from_label else "(from {id: $from_id})"
        ...
        try:
            with self.driver.session() as session:
                ...
                if end_node_id:
                    ...
                    ORDER BY depth
                    """
                    params = {'start_id': start_node_id, 'end_id': end_node_id, 'max_depth': max_depth}
                else:
                    # All reachable nodes
                    query = f"""
                ...
                logger.info(f"Found {len(paths)} paths from {start_node_id} (max depth: {max_depth})")
                return paths
    ...
    def impact_analysis(
        ...
        try:
            with self.driver.session() as session:
                ...
                affected_nodes = []
                for record in result:
                    node_key = 'affected' if analysis_type == 'downstream' else 'dependency' if analysis_type == 'upstream' else 'connected'
